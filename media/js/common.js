...
if( $(document).scrollTop() + $(window).height() > $(document).height() - 10 ) {
		e.preventDefault();
			...
			$.ajax({
			type: "GET",
			url: $('#more a').attr('href') + '#articles',
			dataType: "html",
			success: function(out) {
				result = $(out).find('#articles .post');
				nextlink = $(out).find('#more a').attr('href');
	                    $("#articles").append(result.fadeIn(500));
	                    $('#more a').html('查  看  更  多');
				if (nextlink != undefined) {
					$('#more a').attr('href', nextlink);
				} else {
					$('#more a').html("已经翻到底啦").removeAttr("href")
				}
			}
		});
}
...
$(".friends_list a").each(function(e){
 $(this).prepend("<img src=http://statics.dnspod.cn/proxy_favicon/_/favicon?domain="+this.href.replace(/^(http:\/\/[^\/]+).*$/, '$1').replace( 'http://', '' )+" style='margin:0 2px 0;width:12px;height:12px;'>");
});
...
var bgimg = [
	"http://ww2.sinaimg.cn/large/4eda25f5gw1ea6i91dvflj20xc0go0ut.jpg"
	,"http://ww3.sinaimg.cn/large/4eda25f5gw1ea6iavxy8sj20xc0godiz.jpg"
	,"http://ww1.sinaimg.cn/large/4eda25f5gw1ea6ib80pfoj20xc0goq7h.jpg"
	];
bgimg.sort(function(){return Math.random()>0.5?-1:1;});
$("#head").backstretch(bgimg, {fade: 3000, duration: 10000});
